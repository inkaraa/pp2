...
def generate_food(snake_body):
    ...
    while not valid_position:
        ...
        if [food_x, food_y] not in snake_body:
            valid_position = True
        value = random.choice([1, 2, 3, 5])
        spawn_time = time.time()
    return food_x, food_y, value, spawn_time
...
food_x, food_y, value, spawn_time = generate_food(snake_body)
score = 0
level = 1
food_timeout = 5
...
while running:
    ...
    if snake_head[0] == food_x and snake_head[1] == food_y:
        score += value  # Increase score by 1
    ...
    if time.time() - spawn_time > food_timeout:
